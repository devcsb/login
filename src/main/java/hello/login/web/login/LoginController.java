...
//    @PostMapping("/login")
    public String login(@Valid @ModelAttribute LoginForm form, BindingResult bindingResult, HttpServletResponse response) {
        ...
        // 쿠키에 시간 정보를 주지 않으면 세션 쿠키로 쓰인다.(브라우저 종료 시 쿠키 삭제)
...
//    @PostMapping("/login")
    ...
    public String loginV3(@Valid @ModelAttribute LoginForm form, BindingResult bindingResult, HttpServletRequest request) {
        ...
        //request.getSession(true) : 세션이 있으면 기존 세션 반환, 없으면 새 세션 생성해서 반환.
        //request.getSession(false) : 세션이 있으면 기존 세션 반환, 없으면 null 반환
        HttpSession session = request.getSession(); // 기본값 : true 이므로 생략 가능.
